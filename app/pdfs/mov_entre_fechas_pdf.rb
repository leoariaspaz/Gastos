class MovEntreFechasPdf < Prawn::Document
  ...
  def initialize(movimiento_search, view)
    ...
    titulo = ""
    if MovEntreFechasSearch::TIPOS_INFORME.any? { |ti| ti[1] == movimiento_search.tipo_informe }
      titulo = TIPOS_INFORME.select { |ti| ti[1] == 2 }.first[0].downcase
    end
    cuenta = Cuenta.find(movimiento_search.cuenta_id).descripcion
    @file_name = "Movimientos - Por #{titulo}"
    subtitle =  "Del #{format_date(movimiento_search.fecha_desde)} al #{format_date(movimiento_search.fecha_hasta)} - " + 
                "Cuenta: #{cuenta} - " + "Agrupado por #{titulo}"
